...
One graphic is generated for real: a simple evaluation-weighting bar chart,
built with matplotlib from data the app already has. Everything else
(cover hero image, divider images, methodology diagrams, org charts,
programme timelines, project experience matrices) is flagged as a
placeholder for a human -- or a later, more capable version of this tool --
to actually produce.
...
def recommend_graphics(
    ...
    for section in sections:
        ...
        recs.append(GraphicRecommendation(
            ...
            placeholder_text="" if is_generated else f"[DIVIDER IMAGE PLACEHOLDER: {_project_type_hint(section.title)}]",
        ))
        for graphic_name in section.recommended_graphics:
            recs.append(GraphicRecommendation(
                ...
                suggested_placement=section.title,
                source_data_required=_infer_source_data(graphic_name),
                status="User Input Required" if _infer_source_data(graphic_name) != "app-generated" else "Generated",
                placeholder_text=f"[{graphic_name.upper()} PLACEHOLDER]",
            ))
...
def _project_type_hint(section_title: str) -> str:
    return "BRIDGE / ROAD / INFRASTRUCTURE"
