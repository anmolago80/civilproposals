...
#
# Round 3, Part 5c: this constant, and the three gating functions right
# below, used to be the entire implementation -- now they're a thin wrapper
# over modules/artifact_download_gate.py, which carries the same logic as
# plain functions taking the live-session values as parameters instead of
# reading current_user/_access/IS_SAAS_MODE as globals, so the actual
# decision (not just the underlying db.ArtifactEvent row mechanics) is
# unit-testable outside a real page render. See that module's docstring.
FREE_TIER_ARTIFACTS = artifact_download_gate.FREE_TIER_ARTIFACTS
_TRIAL_ARTIFACT_EVENT_SENTINEL_KEY = artifact_download_gate.TRIAL_ARTIFACT_EVENT_SENTINEL_KEY
...
def _free_artifact_already_downloaded(artifact_type: str) -> bool:
    ...
    Thin wrapper over artifact_download_gate.already_downloaded() (Round 3,
    Part 5c) -- see that function's docstring for the full "Audit fix Part
    2b" reasoning, unchanged."""
    return artifact_download_gate.already_downloaded(
        db, current_user.id if current_user else None, artifact_type, _project_is_free_tier())
...
def _mark_free_artifact_downloaded(artifact_type: str) -> None:
    ...
    Thin wrapper over artifact_download_gate.mark_downloaded() (Round 3,
    Part 5c) -- see that function's docstring for the full "Audit fix Part
    2b/2c" reasoning, unchanged. Its return value (True on a genuine
    unexpected database error) becomes the retry-warning flag the download
    button area checks, same as before this was extracted."""
    if artifact_download_gate.mark_downloaded(
            db, current_user.id if current_user else None, artifact_type, _project_is_free_tier()):
...
def _free_artifact_download_blocked(artifact_type: str) -> bool:
    ...
    Thin wrapper over artifact_download_gate.blocked() (Round 3, Part 5c)."""
    is_free_tier = _project_is_free_tier()
    already = _free_artifact_already_downloaded(artifact_type) if is_free_tier else False
    return artifact_download_gate.blocked(is_free_tier, artifact_type, already)
