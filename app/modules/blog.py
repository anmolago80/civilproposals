...
def render_sitemap_xml(posts: list["db.BlogPost"]) -> str:
  ...
  <url>
    ...
    body = "\n".join(entries)
    return _validated_xml(f"""<?xml version="1.0" encoding="UTF-8"?>
...
def _validated_xml(text: str) -> str:
    """Parses the sitemap before it can be published.

    Learned the hard way: a double hyphen inside an XML comment makes the
    whole document unparseable, and Google reports that only as "1 error,
    0 discovered pages" with no hint as to why. A malformed sitemap fails
    silently for weeks. Better to fail loudly at publish time, in the
    editor, where it can actually be fixed."""
    from xml.etree import ElementTree
    try:
        ElementTree.fromstring(text)
    except ElementTree.ParseError as exc:
        raise PublishError(
            f"The generated sitemap isn't valid XML ({exc}). Publishing was stopped so a "
            f"broken sitemap doesn't reach Google. This usually means a post title or slug "
            f"contains something that needs escaping."
        ) from exc
    return text
