...
def check_page_geometry(failures: list[str], files: dict) -> None:
    """Part 6, test 1: each deck's page size is a deliberate, per-deck
    choice (see org_chart_pptx._SLIDE_W/_SLIDE_H, methodology_pptx.py's,
    program_pptx.py's own module-level constants) -- not something any of
    this project's fix rounds are meant to touch. This asserts the actual
    generated file matches that intent, with hardcoded inch values rather
    than importing the same constants the generators use, so an accidental
    change to one of those constants is caught here rather than silently
    validated against itself.

    Deliberately checked against the *standard* sample project only, not
    against any of this suite's own overflow-stress fixtures (fat_stages,
    30 scope items, etc.) -- both org_chart_pptx._grow() and
    program_pptx.py's equivalent last-resort growth path may legitimately
    enlarge the page for an extreme content load (see their own docstrings:
    "should not fire in ordinary use"), and that growth is correct
    behaviour, not a regression this test should flag."""
    import io as _io

    from pptx import Presentation

    _TOLERANCE_EMU = int(0.2 * 914400)
    expected = {
        "Org_Chart.pptx": (13.33, 7.50),
        "Methodology_Table.pptx": (11.69, 8.27),
        "Delivery_Program.pptx": (16.54, 11.69),
    }
    for name, (want_w, want_h) in expected.items():
        if name not in files:
            failures.append(f"[Part6] {name} was not generated by generate_all()")
            continue
        prs = Presentation(_io.BytesIO(files[name]))
        want_w_emu, want_h_emu = int(want_w * 914400), int(want_h * 914400)
        if abs(prs.slide_width - want_w_emu) > _TOLERANCE_EMU:
            failures.append(
                f"[Part6] {name} page width is {prs.slide_width / 914400:.2f}in, "
                f"expected {want_w:.2f}in (+/- 0.2in) -- page sizes are deliberate, out of scope to change")
        if abs(prs.slide_height - want_h_emu) > _TOLERANCE_EMU:
            failures.append(
                f"[Part6] {name} page height is {prs.slide_height / 914400:.2f}in, "
                f"expected {want_h:.2f}in (+/- 0.2in) -- page sizes are deliberate, out of scope to change")


def check_methodology_label_containment(failures: list[str]) -> None:
    """Part 6, test 2: the boardroom-matrix style's rotated row labels
    (KEY TASKS / ENGAGEMENT / OUTCOME / DELIVERABLES, or their Spanish
    equivalents) must stay inside their own row band in the rendered PNG --
    a genuine pixel-level regression test for the Part 5 fix
    (methodology_render._fit_rotated_label), verified independently of the
    fix's own internal extent measurement so a bug in that measurement
    itself couldn't pass unnoticed by re-checking itself.

    Method, on the actual rendered pixels (no OCR, no reuse of the
    renderer's own reported extents):
      1. The row-label fill colour (#DCE6F7) is a fixed, exact hex the four
         label rectangles are drawn in and nothing else in the image
         matches -- the neighbouring content-cell tints and gridline are
         each a different exact hex. Its rightmost occurrence anywhere in
         the image gives the label column's pixel width without needing to
         know axes-to-pixel geometry.
      2. Within that column, scan every pixel row for (a) any fill-colour
         pixel ("this row belongs to a label's rectangle") and (b) any
         ink-coloured pixel ("this row has label text drawn on it" -- the
         label text colour, #0F2A5C, is dark enough that no light UI tint
         in this renderer is anywhere close to it).
      3. Group the fill rows into contiguous bands -- there must be exactly
         four (one per row label); fewer means two adjacent row rectangles
         are touching or have merged, which is itself the shrunk-row class
         of bug this fix guards against.
      4. Assert no ink row exists without fill also being present in that
         same row: ink appearing where its own row's rectangle doesn't
         reach is exactly what "the label crosses a band boundary" looks
         like on the actual pixels -- textually, letters from one row's
         label bleeding into the gap toward (or into) a neighbour's band.

    Confirmed (by temporarily reverting the fix to its pre-Part-5 hardcoded
    fontsize=5.6 during development of this test) that this correctly
    fails -- ink rows with no fill present -- on the exact class of input
    that produced the originally reported overlap, and passes clean on the
    real fix at 3, 4, 5 and 6 stages in both languages."""
    import io as _io

    import numpy as np
    from PIL import Image

    from modules import methodology_render as mr
    from modules.methodology_stages import MethodologyStage

    _FILL = np.array([0xDC, 0xE6, 0xF7])
    _INK = np.array([0x0F, 0x2A, 0x5C])

    base_stages = [
        MethodologyStage(
            name=f"Stage {i + 1}", week_start=i * 3 + 1, week_end=i * 3 + 3,
            key_tasks=[f"Task {j + 1} for stage {i + 1} covering scope description" for j in range(4)],
            engagement_activities=(["Client workshop", "Hold point sign-off review"]
                                    if i == 1 else ["Design coordination meeting"]),
            outcome="Outcome achieved and documented for client sign-off." if i % 2 == 0 else "TBC",
    ...
    for language in (None, "es"):
        for n in (3, 4, 5, 6):
            stages = base_stages[:n]
            png = mr.render_png(
                None, style="matrix", stages=stages, project_name="Sample Project", language=language)
            label = f"[Part6] matrix label containment (n={n} stages, language={language!r})"
            if png is None:
                failures.append(f"{label}: render_png returned None")
                continue

            arr = np.array(Image.open(_io.BytesIO(png)).convert("RGB"))
            is_fill = np.all(np.abs(arr.astype(int) - _FILL) <= 2, axis=-1)
            is_ink = np.all(np.abs(arr.astype(int) - _INK) <= 60, axis=-1)

            fill_cols = np.nonzero(is_fill.any(axis=0))[0]
            if fill_cols.size == 0:
                failures.append(f"{label}: no row-label fill colour found at all -- labels didn't render")
                continue
            left, right = int(fill_cols.min()), int(fill_cols.max())

            fill_row = is_fill[:, left:right + 1].any(axis=1)
            ink_row = is_ink[:, left:right + 1].any(axis=1)
            fill_ys = np.nonzero(fill_row)[0]
            y0, y1 = int(fill_ys.min()), int(fill_ys.max())
            ...
            for y in range(y0, y1 + 1):
                if fill_row[y]:
                    ...
                    in_segment = False
                    if ink_row[y]:
                        bad_ink_rows.append(y)

            if segments != 4:
                failures.append(
                    f"{label}: found {segments} row-label band(s) in the rendered image, expected 4 -- "
                    "two adjacent row bands have touched or merged")
            if bad_ink_rows:
                failures.append(
                    f"{label}: label text ink found in {len(bad_ink_rows)} row(s) outside any label's own "
                    f"band (e.g. pixel row {bad_ink_rows[0]}) -- a label is crossing into a neighbouring band")
...
def main() -> int:
    ...
    check_page_geometry(failures, files)
    check_methodology_label_containment(failures)
