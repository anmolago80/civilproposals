...
with st.sidebar:
    ...
    st.divider()
    st.caption(f"Format: **{PROPOSAL_FORMAT_LABELS[st.session_state.proposal_format]}**")
    st.subheader("Progress")
    if _is_letter():
        checklist = [
            ("Project details entered", bool(st.session_state.tender_name)),
            ("Tender brief uploaded", st.session_state.tender_extracted is not None),
            ("AI provider configured", bool(st.session_state.ai_config.get("api_key"))),
            ("Tender analysed", st.session_state.analysis is not None),
            ("Proposal structure generated", st.session_state.sections is not None),
            ("Project Understanding / Terms drafted", bool(st.session_state.drafts)),
            ("Team members added", any((a.person_name or "").strip() for a in st.session_state.resource_plan)),
            ("Fees & program set", bool(st.session_state.scope_item_fees)),
            ("Pack exported", st.session_state.docx_buffer is not None),
        ]
    else:
        checklist = [
            ("Project details entered", bool(st.session_state.tender_name)),
            ...
            ("Tender analysed", st.session_state.analysis is not None),
            ("Proposal structure generated", st.session_state.sections is not None),
            ("Drafts generated", st.session_state.drafts is not None),
            ("Graphics reviewed", st.session_state.graphics is not None),
            ("Fee estimate reviewed", st.session_state.fee_estimates is not None),
            ("Pack exported", st.session_state.docx_buffer is not None),
        ]
...
with tabs[8]:
    ...
    if _is_letter():
        ...
        if not scope_items:
            ...
            fee_by_title = {f.item_title: f for f in st.session_state.scope_item_fees}
            fee_rows = [
                {
                    "item_title": item.title,
                    "fee_amount": fee_by_title[item.title].fee_amount if item.title in fee_by_title else 0.0,
                    "notes": fee_by_title[item.title].notes if item.title in fee_by_title else "",
                }
            ...
            edited_fees = st.data_editor(
                ...
                column_config={
                    "item_title": st.column_config.TextColumn("Scope item", disabled=True),
            ...
            st.session_state.scope_item_fees = [
                fee_estimation_engine.ScopeItemFee(
                    item_title=r["item_title"], fee_amount=float(r["fee_amount"] or 0), notes=r["notes"] or "",
                )
